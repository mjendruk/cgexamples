...
void Particles::initialize()
{
   ...
   m_computeShadersAvailable = glbinding::ContextInfo::supported({ gl::GLextension::GL_ARB_compute_shader });
    ...
    glPointSize(m_scale * 2.f / glm::sqrt(glm::pi<float>()));
...
float Particles::elapsed()
{
    ...
    std::cout << elapsedSinceLast << " " << m_elapsedSinceEpoch << std::endl;
...
void Particles::processComputeShaders(float elapsed)
{
    ...
    std::cout << "#  " << elapsed << " ^2 " << elapsed2 << "  epoch " << m_elapsedSinceEpoch <<  std::endl;
...
void Particles::render()
{
    ...
    else if (m_measureCount == 1000)
    {
        const auto measureEnd = m_time;

        std::cout << (1000.0f / (std::chrono::duration_cast<std::chrono::milliseconds>(measureEnd - m_startMeasuring).count() / 1000.0f)) << " FPS" << std::endl;
